...
export const isTokenWhitelisted = async (asset: Asset) => {
  ...
  const isWhitelisted = tokenList1Inch.tokens.find(
    (data: TokenType) =>
      data.address.toLowerCase() === assetAddress.toLowerCase(),
  )
